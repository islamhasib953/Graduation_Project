...
//   if (!vaccine) {
//     return next(
//       appError.create("Vaccine not found", 404, httpStatusText.FAIL)
//     );
//   }
...
//     0,
//     Math.floor((new Date(actualDate) - new Date(dueDate)) / (1000 * 60 * 60 * 24)) + 1
...
const createVaccinationForAllChildren = asyncWrapper(async (req, res, next) => {
  const { disease, originalSchedule } = req.body;

  if (!disease || !originalSchedule) {
    return next(
      appError.create(
        "Disease and original schedule are required",
        400,
        httpStatusText.FAIL
      )
    );
  ...
  const existingVaccine = await VaccineInfo.findOne({ disease });
  if (existingVaccine) {
    return next(
      appError.create("Vaccine already exists", 400, httpStatusText.FAIL)
    );
  }
  ...
  const vaccinationsToCreate = children.map((child) => ({
    ...
    dueDate: new Date(child.birthDate).setMonth(
...
const updateUserVaccination = asyncWrapper(async (req, res, next) => {
  ...
  if (dueDate && dueDate !== vaccination.dueDate.toISOString().split("T")[0]) {
    changes.push(`due date to ${dueDate}`);
    vaccination.dueDate = dueDate;
  }
  if (req.file) {
...
const deleteUserVaccination = asyncWrapper(async (req, res, next) => {
  ...
  await sendNotification(
    ...
    null,
    "Vaccination Deleted",
    `${child.name}: Vaccination deleted.`,
    "vaccination",
    "patient"
  );
