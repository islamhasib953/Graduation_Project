...
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:provider/provider.dart';
import 'package:segma/screens/doctor/profile_screen.dart';
import 'package:segma/services/doctor_service.dart';
import 'package:segma/services/user_service.dart';
import 'package:segma/screens/login_screen.dart';
import 'package:segma/utils/colors.dart';
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  void _showDeleteAccountDialog(BuildContext context) {
    final TextEditingController deleteController = TextEditingController();
    final scaffoldMessenger = ScaffoldMessenger.of(context);
    bool isDialogDeletePressed = false;
          ...
          builder: (dialogContext, setDialogState) {
            return AlertDialog(
              ...
              actions: [
                ...
                GestureDetector(
                  onTapDown: (_) => setDialogState(() => isDialogDeletePressed = true),
                  onTapUp: (_) {
                    setDialogState(() => isDialogDeletePressed = false);
                    if (deleteController.text.toUpperCase() == 'DELETE') {
                      _deleteAccount(context, scaffoldMessenger);
                    }
                  },
                  onTapCancel: () => setDialogState(() => isDialogDeletePressed = false),
                  child: AnimatedScale(
                    scale: isDialogDeletePressed ? 0.95 : 1.0,
                    duration: const Duration(milliseconds: 200),
                    child: ElevatedButton(
                      onPressed: deleteController.text.toUpperCase() == 'DELETE'
                          ? () => _deleteAccount(context, scaffoldMessenger)
