...
class ParentInfoScreen extends StatefulWidget {
  ...
  const ParentInfoScreen({super.key, required this.child});
...
class _ParentInfoScreenState extends State<ParentInfoScreen> {
  ...
  Future<void> _loadUserData() async {
    ...
    try {
      ...
      if (result['status'] == 'success') {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(errorMessage!), action: SnackBarAction(label: 'إعادة المحاولة', onPressed: _loadUserData), duration: const Duration(seconds: 5)),
        );
      }
    } catch (e) {
      print('🔥 Error loading user data: $e');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Parents Info', style: TextStyle(fontFamily: 'Roboto', fontSize: 20.sp)),
        backgroundColor: Theme.of(context).scaffoldBackgroundColor,
        elevation: 0,
      ),
      body: isLoading
                  ...
                  child: Column(
                    ...
                    children: [
                      Text(errorMessage ?? 'حدث خطأ أثناء جلب البيانات',
                          style: TextStyle(fontSize: 16.sp, fontFamily: 'Roboto', color: Colors.red), textAlign: TextAlign.center),
                      SizedBox(height: 16.h),
                      ElevatedButton(
                        onPressed: _loadUserData,
                        style: ElevatedButton.styleFrom(
                        ...
                        child: Text('إعادة المحاولة', style: TextStyle(fontSize: 14.sp, fontFamily: 'Roboto')),
                      ),
                    ],
                  ),
                )
                          ...
                          children: [
                            ClipOval(
                              child: widget.child.photo != null
                                  ? Image.network(widget.child.photo!, width: 60.r, height: 60.r, fit: BoxFit.cover,
                                      errorBuilder: (context, error, stackTrace) => Container(
                                          width: 60.r,
                                          height: 60.r,
                            ...
                            Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Text(widget.child.name, style: TextStyle(fontSize: 18.sp, fontWeight: FontWeight.bold, fontFamily: 'Roboto')),
                                SizedBox(height: 4.h),
                                Row(
                                  children: [
                                    Icon(Icons.cake, size: 16.sp, color: Colors.grey),
                                    SizedBox(width: 4.w),
                                    Text(formattedBirthDate, style: TextStyle(fontSize: 14.sp, color: Colors.grey, fontFamily: 'Roboto')),
                                  ],
                                ),
                              ],
                            ),
                        ...
                        Container(
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  Icon(userGender == 'male' ? Icons.male : Icons.female,
                                      size: 20.sp, color: userGender == 'male' ? Colors.blue : Colors.pink),
                                  SizedBox(width: 8.w),
                                  Text(userGender == 'male' ? 'Father' : userGender == 'female' ? 'Mother' : 'Parent',
                                      style: TextStyle(fontSize: 16.sp, fontWeight: FontWeight.bold, fontFamily: 'Roboto')),
                                ],
                              ),
                              SizedBox(height: 8.h),
                              if (userFirstName != null && userLastName != null)
                                Text('$userFirstName $userLastName', style: TextStyle(fontSize: 14.sp, fontFamily: 'Roboto')),
                              if (userPhone != null) ...[
                                SizedBox(height: 4.h),
                                Text(userPhone!, style: TextStyle(fontSize: 14.sp, fontFamily: 'Roboto')),
                              ],
                            ],
                          ),
                        ),
                            ...
                            child: Column(
                              ...
                              children: additionalInfoList.map((info) {
                                return Padding(
                                  ...
                                  child: Row(
                                    children: [
                                      Icon(Icons.info, size: 20.sp, color: Colors.grey),
                                      SizedBox(width: 8.w),
                                      Expanded(child: Text(info, style: TextStyle(fontSize: 14.sp, fontFamily: 'Roboto'))),
                                    ],
                                  ),
                                );
                              }).toList(),
                        ...
                        if (!isAddingInfo)
                          OutlinedButton(
                            ...
                            child: Row(
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                Icon(Icons.add, size: 20.sp, color: Colors.blue),
                                SizedBox(width: 8.w),
                                Text('Add Parent Info', style: TextStyle(fontSize: 14.sp, color: Colors.blue, fontFamily: 'Roboto')),
                              ],
                            ),
                          )
                        else
                              ...
                              children: [
                                ...userInfoMap.entries.map((entry) {
                                  ...
                                  return Padding(
                                    ...
                                    child: Row(
                                      children: [
                                        Expanded(child: Text('$label: $value', style: TextStyle(fontSize: 14.sp, fontFamily: 'Roboto'))),
                                        IconButton(
                                          onPressed: () => _addInfo('$label: $value'),
                                          icon: Icon(Icons.add_circle, color: Colors.blue, size: 20.sp),
                                        ),
                                      ],
                                    ),
                                  );
                                ...
                                OutlinedButton(
                                  ...
                                  child: Text('Cancel', style: TextStyle(fontSize: 14.sp, color: Colors.red, fontFamily: 'Roboto')),
                                ),
                              ],
                            ),
                          ),
