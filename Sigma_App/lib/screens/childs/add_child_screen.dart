...
class _AddChildScreenState extends State<AddChildScreen> {
  ...
  String _bloodType = 'A+';
  bool _isLoading = false;
  bool _isFetchingGrowthData = false;
  String? _errorMessage;
  bool _isButtonPressed = false;
  bool _isEditMode = false;
  String? _photoUrl;
    ...
    if (pickedFile != null) {
      if (io.Platform.isAndroid || io.Platform.isIOS) {
        setState(() {
          ...
          _photoUrl = null; // سيتم تحديثه بعد الرفع
        });
      } else if (kIsWeb) {
  ...
  Future<void> _addChild() async {
    ...
    final response = _isEditMode
        ? await ChildService.updateChild(child, _selectedImage, context)
        : await ChildService.addChild(child, _selectedImage, context);
                ...
                child: Column(
                  children: [
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      onChanged: (value) => setState(() => _gender = value!),
                      items: [
                        DropdownMenuItem(value: 'Boy', child: Text('Boy', style: Theme.of(context).textTheme.bodyLarge)),
                        DropdownMenuItem(value: 'Girl', child: Text('Girl', style: Theme.of(context).textTheme.bodyLarge)),
                      ],
                      decoration: InputDecoration(
                        labelText: 'Gender',
                        enabledBorder: OutlineInputBorder(
                    ...
                    GestureDetector(
                      ...
                      child: AbsorbPointer(
                        child: TextFormField(
                          controller: TextEditingController(text: DateFormat('yyyy-MM-dd').format(_birthDate)),
                          style: Theme.of(context).textTheme.bodyLarge,
                          decoration: InputDecoration(
                            labelText: 'Birth Date',
                            enabledBorder: OutlineInputBorder(
                    ...
                    if (_errorMessage != null)
                      Padding(
                        ...
                        child: Text(
                          _errorMessage!,
                          style: Theme.of(context).textTheme.bodyLarge?.copyWith(color: AppColors.statusOverdue),
                        ),
                      ),
                    GestureDetector(
                      onTapDown: (_) => setState(() => _isButtonPressed = true),
