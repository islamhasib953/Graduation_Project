...
class _SelectChildScreenState extends State<SelectChildScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Select Child', style: Theme.of(context).textTheme.titleLarge),
        leading: IconButton(icon: Icon(Icons.arrow_back), onPressed: () => Navigator.pop(context)),
      ),
      body: _isLoading
          ? Center(
              child: CircularProgressIndicator(
                color: Theme.of(context).brightness == Brightness.light ? AppColors.lightButtonPrimary : AppColors.darkButtonPrimary,
              ),
            )
          : _error != null
              ? Center(
                  child: Text(_error!, style: Theme.of(context).textTheme.bodyLarge?.copyWith(color: AppColors.statusOverdue), textAlign: TextAlign.center),
                )
              : _children.isEmpty
                  ? Center(
                      child: Text('No children found. Please add a child.',
                                    ...
                                    children: [
                                      CircleAvatar(
                                        ...
                                        child: child.photo == null
                                            ? Icon(Icons.person, size: 40.sp, color: Theme.of(context).iconTheme.color)
                                            : null,
                                      ),
                                      SizedBox(height: 10.h),
                                      Text(
