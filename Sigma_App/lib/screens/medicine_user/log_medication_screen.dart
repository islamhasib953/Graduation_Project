...
class _LogMedicationScreenState extends State<LogMedicationScreen> {
  ...
  Future<void> _scheduleLocalNotifications(Medication medication) async {
    try {
      final now = DateTime.now();
      final location = tz.getLocation('Africa/Cairo'); // استبدل بالمنطقة الزمنية المناسبة
      for (var time in medication.times) {
        for (var day in medication.days) {
          final dayIndex = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'].indexOf(day);
          final nextOccurrence = _getNextOccurrence(now, dayIndex, time.hour, time.minute);
          final scheduledDate = tz.TZDateTime.from(nextOccurrence, location);
          await NotificationService.scheduleLocalNotification(
            id: (medication.id + day + time.toString()).hashCode, // Unique ID for each notification
      ...
      print('✅ Local notifications scheduled successfully for ${medication.name}');
    } catch (e) {
      print('🔥 Error scheduling local notifications: $e');
    }
  }

  DateTime _getNextOccurrence(DateTime now, int targetDayIndex, int hour, int minute) {
    final currentDayIndex = now.weekday % 7; // Sunday = 0, Monday = 1, ..., Saturday = 6
    int daysUntilNext = (targetDayIndex - currentDayIndex + 7) % 7;
    if (daysUntilNext == 0 && (now.hour > hour || (now.hour == hour && now.minute >= minute))) {
      daysUntilNext = 7; // Schedule for next week if time has passed today
    }
    return DateTime(now.year, now.month, now.day, hour, minute).add(Duration(days: daysUntilNext));
  }
  ...
  Future<void> _saveMedication() async {
    ...
    if (medicationCubit.error == null) {
      // Schedule local notifications after successful save
      await _scheduleLocalNotifications(medication);
      await medicationCubit.fetchMedications(widget.childId);
      if (mounted) {
        Navigator.pop(context);
        ScaffoldMessenger.of(context).showSnackBar(
