...
class FavoriteMemoriesScreen extends StatefulWidget {
  ...
  const FavoriteMemoriesScreen({super.key, required this.childId});
            ...
            return Center(
              child: Column(
                ...
                children: [
                  ...
                  Text('Error loading favorite memories', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontSize: 20.sp)),
                  SizedBox(height: 8.h),
                  Text(state.message, style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontSize: 16.sp), textAlign: TextAlign.center),
                  SizedBox(height: 16.h),
                  ElevatedButton(onPressed: () => context.read<MemoryCubit>().loadFavoriteMemories(widget.childId), child: const Text('Retry')),
                ],
              ),
            );
          }
            ...
            return Center(
              child: FadeTransition(
                ...
                child: Column(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    Icon(Icons.favorite_border, size: 60, color: Theme.of(context).colorScheme.primary),
                    SizedBox(height: 16.h),
                    Text('No favorite memories yet!', style: Theme.of(context).textTheme.titleLarge?.copyWith(fontSize: 20.sp)),
                    SizedBox(height: 8.h),
                    Text('Mark some memories as favorites to see them here.', style: Theme.of(context).textTheme.bodyMedium?.copyWith(fontSize: 16.sp), textAlign: TextAlign.center),
                  ],
                ),
              ),
            );
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return FadeTransition(
                opacity: _fadeAnimation,
                child: FavoriteMemoryCard(memory: memory, onTap: () {
                  Navigator.push(
                    context,
...
class FavoriteMemoryCard extends StatelessWidget {
  ...
  const FavoriteMemoryCard({super.key, required this.memory, required this.onTap, required this.onToggleFavorite});
