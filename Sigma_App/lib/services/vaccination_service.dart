...
static Future<Map<String, dynamic>> logVaccination({
  ...
  try {
    ...
    final dateStr = "${actualDate.toIso8601String().split('T')[0]}";
    final timeStr = "${actualDate.hour.toString().padLeft(2, '0')}:${actualDate.minute.toString().padLeft(2, '0')}";

    print('Sending PATCH request for childId: $childId, userVaccinationId: $userVaccinationId');
    print('Request Data: status=$status, actualDate=$dateStr, actualTime=$timeStr, notes=$notes, image=$image');
    ...
    final result = jsonDecode(response.body);
    return result; // إرجاع الاستجابة مباشرة دون استدعاء إضافي
  } catch (e) {
    print('Error in logVaccination: $e');
    return {'status': 'error', 'message': 'Error: $e'};
  }
}
